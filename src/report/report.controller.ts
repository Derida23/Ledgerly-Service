...
export class ReportController {
  ...
  @Get('dashboard')
  @ApiOperation({
    summary: 'Dashboard: total saldo semua wallet + tren 12 bulan',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    name: 'date',
    required: false,
    description: 'Tanggal dalam minggu (default: minggu ini)',
  })
  @ApiResponse({
    status: 200,
  ...
  getWeekly(@CurrentUser() user: { id: string }, @Query('date') date?: string) {
  ...
  @ApiQuery({
    name: 'date',
    required: false,
    description: 'Tanggal dalam bulan (default: bulan ini)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    name: 'date',
    required: false,
    description: 'Tanggal dalam tahun (default: tahun ini)',
  })
  @ApiResponse({
    status: 200,
  ...
  getYearly(@CurrentUser() user: { id: string }, @Query('date') date?: string) {
